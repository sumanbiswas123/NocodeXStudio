...
import { Undo, Redo, Code, Eye, Save, MonitorSmartphone, FolderOpen, Smartphone, Monitor, MousePointer2, Play, Search, FilePlus } from 'lucide-react';
    ...
    >
      {items.map((item, i) => (
        <button
          ...
          onMouseEnter={(e) => {
            if (!item.active) e.currentTarget.style.backgroundColor = 'var(--input-bg)';
          }}
          onMouseLeave={(e) => {
...
const Toolbar: React.FC<ToolbarProps> = ({
   ...
   const [contextMenu, setContextMenu] = useState<{ type: 'mobile' | 'desktop'; x: number; y: number } | null>(null);
   ...
   const mobileMenuItems: ContextMenuItem[] = [
     { label: 'iPhone — Dynamic Island', value: 'dynamic-island', active: mobileFrameStyle === 'dynamic-island', icon: '📱' },
     { label: 'Android — Punch Hole', value: 'punch-hole', active: mobileFrameStyle === 'punch-hole', icon: '🤖' },
   ];

   const desktopMenuItems: ContextMenuItem[] = [
     { label: '1080p  (1920 × 1080)', value: '1080p', active: desktopResolution === '1080p', icon: '🖥️' },
     { label: '1.5K   (2560 × 1440)', value: '1.5k', active: desktopResolution === '1.5k', icon: '🖥️' },
     { label: '2K     (2560 × 1440)', value: '2k', active: desktopResolution === '2k', icon: '🖥️' },
     { label: '4K     (3840 × 2160)', value: '4k', active: desktopResolution === '4k', icon: '🖥️' },
     { label: 'Resizable', value: 'resizable', active: desktopResolution === 'resizable', icon: '↔️' },
   ];
   ...
   return (
      ...
      <div className="h-16 glass-panel border-b-0 flex items-center justify-between px-6 z-20 shrink-0">
         <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-xl text-slate-100 tracking-tight hidden md:block">Nocode-X <span className="text-indigo-400">Studio</span></h1>
         </div>
            ...
            {viewMode === 'edit' && (
               <div className="flex items-center gap-1 bg-slate-900/50 p-1 rounded-lg border border-slate-700/50">
                  <button
                     ...
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${interactionMode === 'edit'
                        ? 'bg-indigo-600 text-white shadow-sm'
                        : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'
                        }`}
                  >
                     <MousePointer2 size={16} /> <span className="hidden lg:inline">Edit</span>
                  </button>
                  <button
                     onClick={() => setInteractionMode('inspect')}
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${interactionMode === 'inspect'
                        ? 'bg-violet-600 text-white shadow-sm'
                        : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'
                        }`}
                  >
                     <Search size={16} /> <span className="hidden lg:inline">Inspect</span>
                  </button>
                  <button
                     onClick={() => setInteractionMode('preview')}
                     className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${interactionMode === 'preview'
                        ? 'bg-emerald-600 text-white shadow-sm'
                        : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'
                        }`}
                  >
                     <Play size={16} /> <span className="hidden lg:inline">Preview</span>
                  </button>
               </div>
            )}
         </div>
         ...
         <div className="flex items-center gap-3">
            ...
            <button
               ...
               className={`flex items-center gap-2 px-4 py-2 rounded-md font-medium text-sm transition-all border ${viewMode === 'code'
                  ? 'bg-slate-800 text-white border-slate-700 shadow-inner'
                  : 'glass-button'
                  }`}
            >
               {viewMode === 'code' ? <><Eye size={16} /> Visual</> : <><Code size={16} /> Code</>}
            </button>
